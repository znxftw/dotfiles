...
if is_directory "${app_path}"; then
  local found=$(osascript -e 'tell application "System Events" to get the name of every login item' | \grep -i "${1}" || true)
  if ! is_non_zero_string "${found}"; then
    osascript -e "tell application \"System Events\" to make login item at end with properties {path:\"${app_path}\", hidden:false}" 2>&1 &> /dev/null && success "Successfully setup '$(yellow "${1}")' as a login item" || warn "Failed to setup '$(yellow "${1}")' as a login item"
  fi
  unset found
else
